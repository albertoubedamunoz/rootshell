...
enum MuxSessionDetach {
    ...
    /// `.muxDetach`. Close Tab also leaves zmx running (native zmx semantics).
    ...
    enum Kind: Equatable {
        ...
        /// `HerdrController.detach` — ends control mode, session stays up.
    ...
    static func attachment(on terminal: Ghostty.TerminalView) -> Attachment? {
        ...
        if let controller = HerdrController.controller(for: terminal), controller.isActive {
            let name = controller.sessionName
            return Attachment(
                kind: .herdrControlMode,
                sessionName: name,
                displayName: displayName(for: .herdr, sessionName: name)
            )
        }
    ...
    static func attachment(
        ...
        for view in tab.splitTree.terminalLeaves {
            if let attachment = attachment(on: view),
               ...
               attachment.kind != .herdrControlMode {
                return attachment
            }
        }
    ...
    static func detach(on terminal: Ghostty.TerminalView) -> Outcome {
        ...
        case .herdrControlMode:
            // Banner is posted inside HerdrController.detach.
            HerdrController.controller(for: terminal)?.detach(closeGateway: false)
            return .detached(attachment)
        case .keySequence(let type):
    ...
    static func performKeySequenceDetach(
        ...
        let steps = keySteps(for: type, on: terminal)
        ...
        Task { @MainActor [weak terminal] in
            ...
            // it for `zmx attach <name>` on reconnect.
            if steps.isEmpty {
    ...
    /// Bytes to type, in order. Prefers swipe-discovery bindings, then
    /// built-in defaults. Empty means “close the local client only” (zmx).
    static func keySteps(for type: MultiplexerType, on terminal: Ghostty.TerminalView? = nil) -> [Data] {
        if let discovered = discoveredSteps(for: type, on: terminal) {
            return dataSteps(from: discovered)
        }
        return defaultKeySteps(for: type)
    }

    /// Built-in detach chords used when swipe discovery has not learned
    /// the host's bindings.
    static func defaultKeySteps(for type: MultiplexerType) -> [Data] {
        switch type {
        case .tmux:
    ...
    private static func discoveredSteps(
        for type: MultiplexerType,
        on terminal: Ghostty.TerminalView?
    ) -> [SequenceStep]? {
        guard let bindings = terminal?.discoveredMultiplexerSwipeBindings else { return nil }
        switch type {
        case .tmux:
            return bindings.tmuxDetachClient
        case .zellij:
            return bindings.zellijDetach
        case .herdr, .zmx:
            return nil
        }
    }

    private static func dataSteps(from steps: [SequenceStep]) -> [Data] {
        steps.map { $0.terminalData() }.filter { !$0.isEmpty }
    }
