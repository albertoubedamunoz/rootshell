...
/// over Unix domain sockets in App Group container.
///
...
struct AppGroupHelper {
    /// Mac-style Team-ID-prefixed group: any binary signed by the team gets silent
    /// access under macOS 15+ group-container protection, including the disclaimed
    /// bare helper, which can never carry the provisioning profile an iOS-style
    /// "group.*" identifier would require.
    static let groupIdentifier = "D97ZME3ET2.rootshell"

    /// Legacy iOS-style group where helper sockets lived before the TCC fix.
    /// Only the app (profile-authorized) may touch it, for migration cleanup.
    nonisolated static let legacyGroupIdentifier = "group.com.kk2.ghostty"
    ...
    static var containerURL: URL? {
        guard let url = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: overrideGroupIdentifier ?? groupIdentifier) else {
            return nil
        }
        // Only entitled processes get the directory auto-created; for the
        // Team-ID group both peers are unentitled, so create it ourselves.
