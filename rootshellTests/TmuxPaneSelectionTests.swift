...
final class TmuxPaneSelectionTests: XCTestCase {
    func testSwapExcludesSourceAndKeepsDisplayOrder() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: [42, 7, 91], excludingPaneID: 7))
    ...
    func testTwoPaneSwapHasOneSelectableTarget() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: [42, 7], excludingPaneID: 42))
        XCTAssertEqual(selection.labels, ["1"])
        XCTAssertEqual(selection.consume("1"), .selected(7))
    }

    func testMissingSwapSourceIsRejected() {
        XCTAssertNil(TmuxPaneSelection(paneIDs: [42, 7], excludingPaneID: 91))
    }
    ...
    func testSwapUsesSameCancellationRulesIncludingEscape() throws {
        ...
        var modified = try XCTUnwrap(TmuxPaneSelection(paneIDs: [42, 7], excludingPaneID: 42))
        XCTAssertEqual(modified.consume("1", modified: true), .cancelled)
    }

    func testSwapPaddingUsesTargetCountNotTotalPaneCount() throws {
        let nine = try XCTUnwrap(TmuxPaneSelection(paneIDs: Array(0..<10), excludingPaneID: 3))
        XCTAssertEqual(nine.labels, (1...9).map(String.init))
        var ten = try XCTUnwrap(TmuxPaneSelection(paneIDs: Array(0..<11), excludingPaneID: 3))
    ...
    func testSwapCommandPreservesFocusZoomAndScopesBothTargets() throws {
        XCTAssertEqual(try TmuxPaneSelection.swapCommand(windowID: 4, sourcePaneID: 17, targetPaneID: 29),
                       "swap-pane -d -Z -s @4.%17 -t @4.%29")
        for (window, source, target) in [(-1, 1, 2), (0, -1, 2), (0, 1, -1), (0, 1, 1)] {
            XCTAssertThrowsError(try TmuxPaneSelection.swapCommand(windowID: window, sourcePaneID: source, targetPaneID: target))
        }
    }

    func testRejectsSinglePaneDuplicateAndInvalidIDs() {
        for ids in [[], [1], [1, 1], [-1, 2]] {
            XCTAssertNil(TmuxPaneSelection(paneIDs: ids))
        }
    }

    func testLabelsSelectStableIDsInDisplayOrder() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: [42, 7, 91]))
    ...
    func testTenPanesUseUnambiguousPaddedLabels() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: Array(20..<30)))
    ...
    func testHundredPanesRemainUnambiguous() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: Array(0..<100)))
    ...
    func testOtherInputCancelsIncludingPasteAndModifiedDigits() throws {
        for input in ["", "a", "\u{1b}", "\r", "\t", "٣", "１２", "12", "0", "9"] {
            var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: [8, 9]))
    ...
    func testInvalidPrefixCancelsImmediately() throws {
        var selection = try XCTUnwrap(TmuxPaneSelection(paneIDs: Array(0..<12)))
    ...
    func testFinishedSelectionCannotChange() throws {
        ...
        var cancelled = try XCTUnwrap(TmuxPaneSelection(paneIDs: [8, 9]))
    ...
    func testZoomQueriesServerAndScopesEveryTarget() async throws {
        for alreadyZoomed in [false, true] {
            var commands: [String] = []
            try await TmuxPaneSelection.zoom(windowID: 4, paneID: 17) { command in
    ...
    func testInvalidTargetSendsNothing() async {
        for (window, pane) in [(-1, 17), (4, -1)] {
            do {
                try await TmuxPaneSelection.zoom(windowID: window, paneID: pane) { _ in
